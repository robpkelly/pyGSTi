...
def create_circuit_list(*args, **kwargs):
    ...
    loopLists = {}
    loopLocals = {'True': True, 'False': False, 'str': str, 'int': int, 'float': float}
...
def list_strings_lgst_can_estimate(dataset, prepStrs, effectStrs):
    ...
    pre = tuple(effectStrs[0])
    l0 = len(pre)  # the first effect string
    post = tuple(prepStrs[0])
    l1 = len(post)  # the first prep string
...
def filter_circuit(circuit, sslbls_to_keep, new_sslbls=None, idle=()):
    ...
    for lbl in circuit:
        sublbls = []
        pintersect = False  # btwn lbl's sslbls & to-keep
        for sublbl in lbl.components:
            if (sublbl.sslbls is None or set(sublbl.sslbls).issubset(sslbls_to_keep)):  # then keep this comp
