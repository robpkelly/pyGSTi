    ...
    for i, rhoVec in enumerate(rhoVecs):
        ...
        for povm in povms:
            for EVec in povm.values():
                ret[i, j] = _np.vdot(EVec.todense(), rhoVec.todense())
                j += 1
...
def robust_logGTi_and_projections(modelA, modelB, syntheticIdleStrs):
    ...
    for s in syntheticIdleStrs:
        ...
        print("DB: Added synthetic idle %s => rank=%d <?> %d (shape=%s; %s)" % (str(s), rank, nSuperOps * nOperations, str(runningJac.shape), str(runningY.shape)))
    ...
    for combo, val in zip(Vt[0:rank, :], vals):
        combo_str = " + ".join(["%.1f*%s" % (c, errLbl) for c, errLbl in zip(combo, op_error_labels) if abs(c) > COEFF_TOL])
        ret[combo_str] = val
    return ret
