    ...
    if comm is not None:
        ...
        try:
            if model.frobeniusdist(mdl_cmp) > 1e-6:
                raise ValueError("MPI ERROR in gaugeopt: *different* models given to different processors!")  # pragma: no cover
...
def _create_objective_fn(model, targetModel, itemWeights=None,
    ...
    if method == "ls":
        ...
        def _jacobian_fn(mdl_pre, mdl_post, gaugeGroupEl):
            ...
            derivSlices, myDerivColSlice, derivOwners, mySubComm = _mpit.distribute_slice(allDerivColSlice, comm)
