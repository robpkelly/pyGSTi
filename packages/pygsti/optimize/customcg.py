...
def fmax_cg(f, x0, maxiters=100, tol=1e-8, dfdx_and_bdflag=None, xopt=None):
    ...
    solution = _optResult()
    solution.x = x
    solution.fun = -fx if fx is not None else None  # negate maximum to conform to other minimization routines
    if step < maxiters:
...
def _maximize1D(g, s1, s2, g1):
    ...
    if g1 is None or g3 is None:
        if g1 is None: s1, g1 = _findBoundary(g, s3, s1)
        s1_on_bd = True
        if g3 is None: s3, g3 = _findBoundary(g, s1, s3)
        s3_on_bd = True
        s2 = s1 + (s3 - s1) / PHI
        g2 = g(s2)
    ...
    while((abs(s3 - s1) > TOL) and (abs(s3 - s1) > FRAC_TOL * (abs(s3) + abs(s1)))):
        ...
        if g3 > g2:
            if g2 >= g1:  # Expand to the right
                if s3_on_bd: print("** Returning on bd")
                return s3  # can't expand any further to right
                s2, g2 = s3, g3
                s3 = s1 + (s3 - s1) * PHI
                g3 = g(s3)
                if g3 is None:
            ...
            if g2 <= g1:  # Expand to the left
                if s1_on_bd: print("** Returning on bd2")
                return s1  # can't expand any further to left
                s2, g2 = s1, g1
                s1 = s3 - (s3 - s1) * PHI
                g1 = g(s1)
                if g1 is None:
