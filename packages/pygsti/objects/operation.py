                    ...
                    for l in range(j, N):
                        if _np.isclose(conj, self.evals[l]) and \
                                (_np.allclose(conjB, self.B[:, l])
                                 ...
                                 or _np.allclose(conjB, -1j * self.B[:, l])
                                 or _np.allclose(conjB, -1 * self.B[:, l])):  # numpy normalizes but doesn't fix "phase" of evecs
    ...
    def deriv_wrt_params(self, wrtFilter=None):
        ...
        for k, pdesc in enumerate(self.params):
            ...
            if _np.linalg.norm(tmp.imag) >= IMAG_TOL:  # just a warning until we figure this out.
                print("EigenvalueParamDenseOp deriv_wrt_params WARNING:" + " Imag part = ", _np.linalg.norm(tmp.imag), " pdesc = ", pdesc)  # pragma: no cover
    ...
    def __init__(self, stateSpaceLabels, targetLabels, gate_to_embed):
        ...
        if evotype in ("densitymx", "statevec"):
            ...
            if len(set(iTensorProdBlks)) != 1:
                raise ValueError("All qubit labels of a multi-qubit gate must correspond to the" + " same tensor-product-block of the state space -- checked previously")  # pragma: no cover
