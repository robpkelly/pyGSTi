    ...
    def update_probs(self, probs_in, probs_out, freqs, circuits, elIndices):
        ...
        for i, circ in enumerate(circuits):
            ...
            for j, alpha0, beta0, AorB in sorted_breaks:
                ...
                if AorB:  # A
                    Alst = list(A)
                    del Alst[Alst.index(j)]
                    A = _np.array(Alst, int)
                    Clst = list(C)
                    Clst.append(j)
                    C = _np.array(Clst, int)  # move A -> C
                else:  # B
                    Blst = list(B)
                    del Blst[Blst.index(j)]
                    B = _np.array(Blst, int)
                    Clst = list(C)
                    Clst.append(j)
                    C = _np.array(Clst, int)  # move B -> C
...
class PrimitiveOpsWildcardBudget(WildcardBudget):
    ...
    def __init__(self, primitiveOpLabels, start_budget=0.01):
        ...
        Wvec = _np.array([start_budget] * nPrimOps) + start_budget / 10.0 * _np.arange(nPrimOps)  # 2nd term to slightly offset initial values
